...
**Requires FFmpeg and FFprobe.** Check availability first with `ffmpeg -version`; if missing, install via the system package manager (e.g. `apt-get install -y ffmpeg`) before starting.
